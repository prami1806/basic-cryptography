...
def main():
    ...
    if mode=='encrypt':
        ...
        privKeyFilename='pk_privkey.txt'
    ...
    for blockInt in blockInts:
        ...
        for i in range(blockSize-1,-1,-1):
            if len(message)+i<messageLength:
                # Decode the message string for the blockSize
...
def encryptAndWriteToFile(messageFilename,keyFilename,message,blockSize=None):
    ...
    if not(math.log(2**keySize,len(SYMBOLS))>=blockSize):
        sys.exit('ERROR: Block size is too large for the key and the symbol set size. Did you specify the correct key file and encrypted file?')
    # Encrypt the message:
...
# If program is run instead of imported as a module, run main
